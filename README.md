# 👋 Hello, I'm Robson Nunes

🔐 Cybersecurity | Endpoint Management | Microsoft Security

---

## 🧠 About Me

I am a cybersecurity professional specialized in Endpoint Management and Microsoft Security environments.

I have strong hands-on experience with Microsoft 365, Intune, Azure/Entra ID, and security operations, focusing on protecting corporate environments, ensuring compliance, and strengthening Zero Trust strategies.

---

## 🛠️ Technical Skills

- Microsoft 365 Administration
- Microsoft Intune & Endpoint Management
- Azure / Entra ID (IAM & Identity Protection)
- Security & Compliance (MFA, DLP, Conditional Access)
- Vulnerability Management (Tenable, Trend Vision One)
- Endpoint Hardening (DISA STIG / SCAP)
- Incident Response & Threat Investigation
- PowerShell Automation
- KQL (Kusto Query Language)

---

## 🧰 Tools & Technologies

- Microsoft 365 / Azure
- Intune / SCCM
- PowerShell
- KQL
- Trend Micro Vision One
- Tenable Vulnerability Management
- PRTG Monitoring

---

## 📜 Certifications

- SC-900 – Microsoft Security, Compliance, and Identity Fundamentals
- AZ-900 – Microsoft Azure Fundamentals
- Google Cybersecurity Professional Certificate
- Certified CSCSO – SME Cyber Security Officer
- Trend Vision One Certified (Advanced Network Security)
- HDI-SCA
- Lean Six Sigma White Belt & Yellow Belt

📚 Currently studying: **SC-200 (Microsoft Security Operations Analyst)**

---

## 🔍 Areas of Expertise

- Threat Hunting
- Incident Response
- Endpoint Protection
- Identity & Access Management (IAM/PAM)
- Vulnerability Management
- Security Monitoring & Log Analysis

---

## 📂 Featured Projects (in progress)

- 🔹 PowerShell automation for Microsoft 365
- 🔹 KQL queries for threat detection
- 🔹 Endpoint security baselines (STIG/Hardening)
- 🔹 Incident response playbooks

---

## 🌐 Connect with me

🔗 LinkedIn: https://www.linkedin.com/in/robsonsecurity  
📧 Email: robsoncyberdefense@gmail.com
💬 WhatsApp: [Contact me directly](https://wa.me/5521998837215?text=Ol%C3%A1%20Robson,%20vi%20seu%20portif%C3%B3lio%20e%20gostaria%20de%20conversar!)

---

⭐ Always learning. Always improving. Always defending.

---

## 🇧🇷 Sobre mim (Português)

Sou um profissional de Segurança Cibernética com foco em Microsoft 365, Intune, Azure/Entra ID, gestão de endpoints e resposta a incidentes.

Atuo com proteção de ambientes corporativos, hardening de dispositivos, análise de vulnerabilidades e estratégias de Zero Trust.

---

